...
function  [nspop,rank_info]=non_dominant_sort(pop,m,n)
  ...
  for i=1:xsize
      for j=i+1:xsize
          ...
          more=0;
           ...
           if less==0 && equal~=m    %不存在比另外一个值小，且不全部等于的情况下，即为被支配 j支配i
               person(i).n=person(i).n+1;
               person(j).s=[person(j).s i];
           elseif more==0 && equal~=m   %不存在比另外一个值大，且不全部等于的情况下，即为被支配 i支配j
  ...
  while(~isempty(F(rank).f))
      H=[];
      rank_info=[rank_info  length(F(rank).f)]; %为后续拥挤距离的计算做服务
